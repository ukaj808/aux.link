...
export class DropElement {
    ...
    private removeDraggableStyling() {
        this.dropZoneEl.classList.remove('draggable');
    }

    private initSortableList(file: File) {
        this.removeDraggableStyling();
        const songQueueEl = document.createElement('ol');
        songQueueEl.classList.add('song-queue-list', 'full-flex');
