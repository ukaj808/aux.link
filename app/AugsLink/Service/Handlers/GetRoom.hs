...
renderRoomPage room = H.docTypeHtml $ do
  H.head $ do
    ...
    H.style  ! A.id "room-mobile-stylesheet" ! A.media "screen and (min-width:0px) and (max-width:1025px)" $
      H.toHtml $ CSS.render RoomMobile.styles
    H.style ! A.id "room-desktop-stylesheet" ! A.media "screen and (min-width:1025px)" $
      H.toHtml $ CSS.render RoomDesktop.styles
